...
function showWinScreen(winner) {
    ...
    btnShare.style.cssText = `
        ...
        font-size:1.05rem;font-weight:700;letter-spacing:.5px;
        font-family:"Microsoft JhengHei","PingFang TC",sans-serif;
        border:1.5px solid ${isPlayer?'rgba(255,210,80,.5)':'rgba(100,165,255,.45)'};
        background:${isPlayer?'rgba(255,200,50,.13)':'rgba(50,100,220,.13)'};
        color:${isPlayer?'rgba(255,230,140,.95)':'rgba(160,205,255,.95)'};
        transition:background .2s;
    `;
    btnShare.textContent = "📤 分享本局成果";
    btnShare.onclick = () => generateShareCard(isPlayer, winner.n);
...
// 📤 分享圖卡產生器
// =============================================
function generateShareCard(isWin, winnerName) {
    const W = 540, H = 960;
    ...
    bg.addColorStop(1, "#0d3b6e");
    ctx.fillStyle = bg;
    ctx.fillRect(0, 0, W, H);

    // ── 波浪橢圓裝飾（三層）──────────────────────
    [180, 420, 700].forEach((y, i) => {
        ctx.beginPath();
        ctx.ellipse(W / 2, y, W * 0.88, 32 + i * 10, 0, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(100,180,255,${0.07 + i * 0.03})`;
        ctx.lineWidth = 1.2;
        ctx.stroke();
    });
    ...
    ctx.fillText("友魚勇者之路", W / 2, 72);

    // 細分隔線
    ctx.beginPath();
    ctx.moveTo(W * 0.22, 86); ctx.lineTo(W * 0.78, 86);
    ...
    ctx.fillStyle = isWin ? "#FFD060" : "#6BAED6";
    ctx.fillText(isWin ? "✦ 任務達成 ✦" : "再接再厲", W / 2, 155);

    // 勝者名稱（落敗時顯示勝者）
    if (!isWin) {
        ctx.font = `20px "PingFang TC","Microsoft JhengHei",sans-serif`;
        ctx.fillStyle = "rgba(160,205,255,0.85)";
        ctx.fillText(`由 ${winnerName} 率先完成`, W / 2, 192);
    }

    // ── 海洋友善度分數 ────────────────────────────
    const logs = (typeof gameLog !== "undefined") ? gameLog : [];
    const greenCount = logs.filter(g => g.light === 1 && g.success).length;
    const score = greenCount * 10;
    const totalPlayed = logs.filter(g => g.success !== null).length;

    ctx.font = `bold 30px "PingFang TC","Microsoft JhengHei",sans-serif`;
    ctx.fillStyle = "#4CAF50";
    ctx.fillText(`🌊 海洋友善度  ${score} 分`, W / 2, isWin ? 218 : 238);

    // ── 出牌列表標題 ──────────────────────────────
    const listStartY = isWin ? 270 : 290;
    ctx.font = `bold 18px "PingFang TC","Microsoft JhengHei",sans-serif`;
    ctx.fillStyle = "rgba(180,220,255,0.7)";
    ctx.fillText("本局出牌", W / 2, listStartY);

    ctx.beginPath();
    ctx.moveTo(W * 0.15, listStartY + 10); ctx.lineTo(W * 0.85, listStartY + 10);
    ...
    displayLogs.forEach((g, i) => {
        const col = i % 2;
        const row = Math.floor(i / 2);
        const x = col === 0 ? 30 : col2X;
        const y = listStartY + 28 + row * rowH;

        // 列底色
        const rowBg = col === 0
            ? ctx.createLinearGradient(x - 8, y - 18, x + colW - 12, y + 16)
            : ctx.createLinearGradient(x - 8, y - 18, x + colW - 12, y + 16);
        rowBg.addColorStop(0, "rgba(255,255,255,0.05)");
        rowBg.addColorStop(1, "rgba(255,255,255,0.02)");
        ctx.fillStyle = rowBg;
        ctx.beginPath();
        ctx.roundRect(x - 8, y - 20, colW - 4, rowH - 4, 8);
        ctx.fill();

        // 燈號圓點
        const dotColor = g.light === 1 ? "#4CAF50" : g.light === 2 ? "#FFC107" : "#F44336";
        ctx.beginPath();
        ctx.arc(x + 10, y - 4, 7, 0, Math.PI * 2);
        ctx.fillStyle = dotColor;
        ctx.fill();

        // 成功/失敗標記
        const mark = g.success === true ? "✓" : g.success === false ? "✗" : "－";
        const markColor = g.success === true ? "#66BB6A" : g.success === false ? "#EF5350" : "#90A4AE";
        ctx.font = `bold 14px "PingFang TC","Microsoft JhengHei",sans-serif`;
        ctx.fillStyle = markColor;
        ctx.textAlign = "left";
        ctx.fillText(mark, x + 22, y - 1);

        // 魚名
        ctx.font = `bold 17px "PingFang TC","Microsoft JhengHei",sans-serif`;
        ctx.fillStyle = "rgba(220,240,255,0.92)";
        ctx.fillText(g.fishName, x + 36, y);
    });
    ...
    if (logs.length === 0) {
        ...
        ctx.fillText("（本局無出牌紀錄）", W / 2, listStartY + 60);
    }
    ...
    ctx.fillText("#友魚勇者之路  #永續海鮮", W / 2, H - 96);

    // ── 網址 ──────────────────────────────────────
    ctx.font = `15px "PingFang TC","Microsoft JhengHei",sans-serif`;
    ctx.fillStyle = "rgba(130,185,255,0.55)";
    ctx.fillText("友魚勇者之路", W / 2, H - 60);

    // ── 底部細線 ──────────────────────────────────
    ctx.beginPath();
    ctx.moveTo(W * 0.2, H - 44); ctx.lineTo(W * 0.8, H - 44);
    ctx.strokeStyle = "rgba(100,180,255,0.18)"; ctx.lineWidth = 1; ctx.stroke();

    // ── 分享或下載 ────────────────────────────────
    canvas.toBlob(blob => {
        const file = new File([blob], "友魚勇者.png", { type: "image/png" });
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            navigator.share({
                title: "友魚勇者之路",
                text: `海洋友善度 ${score} 分！${isWin ? "任務達成 🏆" : "再接再厲 🌊"} #友魚勇者之路`,
                files: [file]
            }).catch(() => {}); // 使用者取消不報錯
        } else {
            // 桌機 fallback：直接下載
